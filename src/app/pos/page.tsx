    ...
    <div className="w-full space-y-6">
      ...
      {posStep === 'menu' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredProducts.map((prod) => {
                ...
                return (
                  ...
                  >
                    {/* Hình ảnh */}
                    <div className="relative h-44 bg-coffee-light overflow-hidden">
                      {prod.image_url ? (
                        <img
                          src={prod.image_url}
                          alt={prod.name}
                      ...
                      {cartQty > 0 && (
                        <div className="absolute top-3 right-3 bg-coffee-accent text-coffee-dark font-extrabold text-sm w-8 h-8 rounded-full flex items-center justify-center shadow-lg border border-white">
                          {cartQty}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
                      <div>
                        <h4 className="font-bold text-coffee-dark text-sm leading-tight line-clamp-2">{prod.name}</h4>
                        <p className="text-xs text-coffee-medium mt-1 flex items-center">
                          <Tag className="w-3.5 h-3.5 mr-1" />
                          <span>{categories.find(c => c.id === prod.category_id)?.name}</span>
                        </p>
                      </div>

                      <div className="flex items-center justify-between pt-2">
                        <span className="font-extrabold text-base text-coffee-primary">
                          {prod.price.toLocaleString('vi-VN')}đ
                        </span>

                        {isOutOfStock ? (
                          <span className="text-[10px] text-red-500 font-semibold uppercase">Hết hàng</span>
                        ) : cartQty > 0 ? (
                          <div className="flex items-center bg-[#FAF6F0] rounded-xl border border-coffee-accent/40 overflow-hidden shadow-inner">
                            <button
                              onClick={() => updateQuantity(prod.id, -1)}
                              className="p-2 hover:bg-coffee-accent/20 text-coffee-primary transition"
                            >
                              <Minus className="w-3.5 h-3.5" />
                            </button>
                            <span className="px-2.5 font-bold text-xs text-coffee-dark min-w-[20px] text-center">
                              {cartQty}
                            </span>
                            <button
                          ...
                          <button
                            onClick={() => addToCart(prod)}
                            className="px-3.5 py-1.5 bg-coffee-accent hover:bg-coffee-accent/80 text-coffee-dark font-bold text-xs rounded-xl shadow-sm transition"
                          >
                            Thêm
                          </button>
                        )}
          ...
          <div className="bg-white rounded-3xl p-6 shadow-sm border border-coffee-light flex flex-col min-h-[500px]">
